...
static inline double parse_metric_value(const std::string &value_str)
{
    std::string cleaned;
    cleaned.reserve(value_str.size());
    for (char c : value_str)
    {
        // Keep digits, sign, decimal point and exponent markers; drop thousands separators
    ...
    // Use classic C locale to ensure '.' decimal handling
...
std::unordered_map<std::string, kernel_metrics> create_metrics(const std::string &filename)
{
    ...
    int idx_id = -1;
    int idx_kernel_name = -1;
    int idx_section_name = -1;
    int idx_metric_name = -1;
    int idx_metric_value = -1;
    ...
    if (file.is_open())
    {
        ...
        // Skip non-CSV preamble; stop at first line that looks like CSV (starts with a quote)
        while (std::getline(file, line))
        {
            if (!line.empty() && line.front() == '"') break;
        }

        if (!file.good() && (line.empty() || line.front() != '"'))
        {
            std::cout << "Could not find CSV header in file: " << filename << std::endl;
            return {};
        }

        // Parse header with quote-aware CSV
        auto header_fields = parse_csv_line(line);
        for (size_t i = 0; i < header_fields.size(); ++i)
        {
            const std::string &h = header_fields[i];
            if (h == "ID") idx_id = static_cast<int>(i);
            else if (h == "Kernel Name") idx_kernel_name = static_cast<int>(i);
            else if (h == "Section Name") idx_section_name = static_cast<int>(i);
            else if (h == "Metric Name") idx_metric_name = static_cast<int>(i);
            else if (h == "Metric Value") idx_metric_value = static_cast<int>(i);
        }
    ...
    std::unordered_map<std::string, kernel_metrics> metric_map;

    if (idx_kernel_name < 0 || idx_metric_name < 0 || idx_metric_value < 0)
    {
        std::cout << "CSV header missing required columns in: " << filename << std::endl;
        return {};
    }
    ...
    {
        // Validate row width
        if (static_cast<int>(row.size()) <= idx_metric_name || static_cast<int>(row.size()) <= idx_metric_value || static_cast<int>(row.size()) <= idx_kernel_name) continue;
        if (idx_section_name >= 0 && static_cast<int>(row.size()) > idx_section_name)
        {
            if (row[idx_section_name] != "Command line profiler metrics") continue;
        }

        const std::string &kernel_name = row[idx_kernel_name];
        if (kernel_name.empty()) continue;

        // Get or create kernel entry (value-init to zero-initialize metrics)
        auto [it, inserted] = metric_map.try_emplace(kernel_name, kernel_metrics{});
        kernel_metrics &kernel_obj = it->second;
        if (inserted)
        {
            // Prefer "ID" if present, else fallback to Process ID (index 1)
            int id_val = 0;
            if (idx_id >= 0 && static_cast<int>(row.size()) > idx_id)
            {
                try { id_val = std::stoi(row[idx_id]); } catch (...) { id_val = 0; }
            }
            else if (static_cast<int>(row.size()) > 1)
            {
                try { id_val = std::stoi(row[1]); } catch (...) { id_val = 0; }
            }
            kernel_obj.id = id_val;
            kernel_obj.kernel_name = kernel_name;
        }

        const std::string &metric_name = row[idx_metric_name];
        const double value = parse_metric_value(row[idx_metric_value]);

        // Assign parsed metric to the correct field
        if (metric_name == "smsp__sass_inst_executed.sum") { kernel_obj.metrics_list.smsp__sass_inst_executed = value; }
        if (metric_name == "l1tex__t_sectors_pipe_lsu_mem_global_op_st.sum") { kernel_obj.metrics_list.l1tex__t_sectors_pipe_lsu_mem_global_op_st = value; }
        if (metric_name == "smsp__sass_inst_executed_op_global.sum") { kernel_obj.metrics_list.smsp__sass_inst_executed_op_global = value; }
        if (metric_name == "smsp__warps_active.sum") { kernel_obj.metrics_list.smsp__warps_active = value; }
        if (metric_name == "smsp__warp_issue_stalled_barrier_per_warp_active.pct") { kernel_obj.metrics_list.smsp__warp_issue_stalled_barrier_per_warp_active = value; }
        if (metric_name == "smsp__warp_issue_stalled_membar_per_warp_active.pct") { kernel_obj.metrics_list.smsp__warp_issue_stalled_membar_per_warp_active = value; }
        if (metric_name == "smsp__warp_issue_stalled_short_scoreboard_per_warp_active.pct") { kernel_obj.metrics_list.smsp__warp_issue_stalled_short_scoreboard_per_warp_active = value; }
        if (metric_name == "smsp__warp_issue_stalled_wait_per_warp_active.pct") { kernel_obj.metrics_list.smsp__warp_issue_stalled_wait_per_warp_active = value; }
        if (metric_name == "smsp__thread_inst_executed_per_inst_executed.ratio") { kernel_obj.metrics_list.smsp__thread_inst_executed_per_inst_executed = value; }
        if (metric_name == "sm__sass_branch_targets.avg") { kernel_obj.metrics_list.sm__sass_branch_targets = value; }
        if (metric_name == "sm__sass_branch_targets_threads_divergent.avg") { kernel_obj.metrics_list.sm__sass_branch_targets_threads_divergent = value; }
        if (metric_name == "smsp__warp_issue_stalled_imc_miss_per_warp_active.pct") { kernel_obj.metrics_list.smsp__warp_issue_stalled_imc_miss_per_warp_active = value; }
        if (metric_name == "smsp__warp_issue_stalled_long_scoreboard_per_warp_active.pct") { kernel_obj.metrics_list.smsp__warp_issue_stalled_long_scoreboard_per_warp_active = value; }
        if (metric_name == "sm__warps_active.avg.pct_of_peak_sustained_active") { kernel_obj.metrics_list.sm__warps_active = value; }
        if (metric_name == "smsp__warp_issue_stalled_lg_throttle_per_warp_active.pct") { kernel_obj.metrics_list.smsp__warp_issue_stalled_lg_throttle_per_warp_active = value; }
        if (metric_name == "smsp__warp_issue_stalled_mio_throttle_per_warp_active.pct") { kernel_obj.metrics_list.smsp__warp_issue_stalled_mio_throttle_per_warp_active = value; }
        if (metric_name == "smsp__warp_issue_stalled_tex_throttle_per_warp_active.pct") { kernel_obj.metrics_list.smsp__warp_issue_stalled_tex_throttle_per_warp_active = value; }
        if (metric_name == "sm__sass_inst_executed_op_global_red.sum") { kernel_obj.metrics_list.sm__sass_inst_executed_op_global_red = value; }
        if (metric_name == "sm__sass_inst_executed_op_shared_atom.sum") { kernel_obj.metrics_list.sm__sass_inst_executed_op_shared_atom = value; }
        if (metric_name == "l1tex__data_pipe_lsu_wavefronts_mem_shared_op_ld.sum") { kernel_obj.metrics_list.l1tex__data_pipe_lsu_wavefronts_mem_shared_op_ld = value; }
        if (metric_name == "sm__sass_inst_executed_op_shared_ld.sum") { kernel_obj.metrics_list.sm__sass_inst_executed_op_shared_ld = value; }
        if (metric_name == "l1tex__data_pipe_lsu_wavefronts_mem_shared_op_st.sum") { kernel_obj.metrics_list.l1tex__data_pipe_lsu_wavefronts_mem_shared_op_st = value; }
        if (metric_name == "sm__sass_inst_executed_op_shared_st.sum") { kernel_obj.metrics_list.sm__sass_inst_executed_op_shared_st = value; }
        if (metric_name == "smsp__sass_average_data_bytes_per_wavefront_mem_shared.pct") { kernel_obj.metrics_list.smsp__sass_average_data_bytes_per_wavefront_mem_shared = value; }
        if (metric_name == "smsp__inst_executed_op_local_ld.sum") { kernel_obj.metrics_list.smsp__inst_executed_op_local_ld = value; }
        if (metric_name == "smsp__inst_executed_op_local_st.sum") { kernel_obj.metrics_list.smsp__inst_executed_op_local_st = value; }
        if (metric_name == "lts__t_sectors_op_atom.sum") { kernel_obj.metrics_list.lts__t_sectors_op_atom = value; }
        if (metric_name == "lts__t_sectors_op_read.sum") { kernel_obj.metrics_list.lts__t_sectors_op_read = value; }
        if (metric_name == "lts__t_sectors_op_red.sum") { kernel_obj.metrics_list.lts__t_sectors_op_red = value; }
        if (metric_name == "lts__t_sectors_op_write.sum") { kernel_obj.metrics_list.lts__t_sectors_op_write = value; }
        if (metric_name == "l1tex__t_sector_hit_rate.pct") { kernel_obj.metrics_list.l1tex__t_sector_hit_rate = value; }
        if (metric_name == "sm__sass_inst_executed_op_global_ld.sum") { kernel_obj.metrics_list.sm__sass_inst_executed_op_global_ld = value; }
        if (metric_name == "sm__sass_inst_executed_op_local_ld.sum") { kernel_obj.metrics_list.sm__sass_inst_executed_op_local_ld = value; }
        if (metric_name == "l1tex__t_sectors_pipe_lsu_mem_global_op_ld.sum") { kernel_obj.metrics_list.l1tex__t_sectors_pipe_lsu_mem_global_op_ld = value; }
        if (metric_name == "l1tex__t_sector_pipe_lsu_mem_global_op_ld_hit_rate.pct") { kernel_obj.metrics_list.l1tex__t_sector_pipe_lsu_mem_global_op_ld_hit_rate = value; }
        if (metric_name == "lts__t_sector_op_read_hit_rate.pct") { kernel_obj.metrics_list.lts__t_sector_op_read_hit_rate = value; }
        if (metric_name == "l1tex__t_sectors_pipe_lsu_mem_local_op_ld.sum") { kernel_obj.metrics_list.l1tex__t_sectors_pipe_lsu_mem_local_op_ld = value; }
        if (metric_name == "l1tex__t_sector_pipe_lsu_mem_local_op_ld_hit_rate.pct") { kernel_obj.metrics_list.l1tex__t_sector_pipe_lsu_mem_local_op_ld_hit_rate = value; }
        if (metric_name == "l1tex__t_sectors_pipe_lsu_mem_global_op_red.sum") { kernel_obj.metrics_list.l1tex__t_sectors_pipe_lsu_mem_global_op_red = value; }
        if (metric_name == "l1tex__t_sectors_pipe_lsu_mem_global_op_atom.sum") { kernel_obj.metrics_list.l1tex__t_sectors_pipe_lsu_mem_global_op_atom = value; }
        if (metric_name == "l1tex__t_sector_pipe_lsu_mem_global_op_red_hit_rate.pct") { kernel_obj.metrics_list.l1tex__t_sector_pipe_lsu_mem_global_op_red_hit_rate = value; }
        if (metric_name == "l1tex__t_sector_pipe_lsu_mem_global_op_atom_hit_rate.pct") { kernel_obj.metrics_list.l1tex__t_sector_pipe_lsu_mem_global_op_atom_hit_rate = value; }
        if (metric_name == "lts__t_sector_op_red_hit_rate.pct") { kernel_obj.metrics_list.lts__t_sector_op_red_hit_rate = value; }
        if (metric_name == "lts__t_sector_op_atom_hit_rate.pct") { kernel_obj.metrics_list.lts__t_sector_op_atom_hit_rate = value; }
        if (metric_name == "sm__sass_data_bytes_mem_shared_op_atom.sum") { kernel_obj.metrics_list.sm__sass_data_bytes_mem_shared_op_atom = value; }
        if (metric_name == "l1tex__m_xbar2l1tex_read_sectors_mem_lg_op_ld.sum.pct_of_peak_sustained_elapsed") { kernel_obj.metrics_list.l1tex__m_xbar2l1tex_read_sectors_mem_lg_op_ld_bandwidth = value; }
        if (metric_name == "l1tex__average_t_sectors_per_request_pipe_lsu_mem_global_op_ld.ratio") { kernel_obj.metrics_list.l1tex__average_t_sectors_per_request_pipe_lsu_mem_global_op_ld = value; }
        if (metric_name == "smsp__inst_executed_op_global_ld.sum") { kernel_obj.metrics_list.smsp__inst_executed_op_global_ld = value; }
        if (metric_name == "memory_l2_theoretical_sectors_global") { kernel_obj.metrics_list.memory_l2_theoretical_sectors_global = value; }
        if (metric_name == "memory_l2_theoretical_sectors_global_ideal") { kernel_obj.metrics_list.memory_l2_theoretical_sectors_global_ideal = value; }
        if (metric_name == "memory_l1_wavefronts_shared") { kernel_obj.metrics_list.memory_l1_wavefronts_shared = value; }
        if (metric_name == "memory_l1_wavefronts_shared_ideal") { kernel_obj.metrics_list.memory_l1_wavefronts_shared_ideal = value; }
        if (metric_name == "sm__sass_inst_executed_op_texture.sum") { kernel_obj.metrics_list.sm__sass_inst_executed_op_texture = value; }
        if (metric_name == "l1tex__t_sectors_pipe_tex_mem_texture.sum") { kernel_obj.metrics_list.l1tex__t_sectors_pipe_tex_mem_texture = value; }
        if (metric_name == "l1tex__t_sector_pipe_tex_mem_texture_op_tex_hit_rate.pct") { kernel_obj.metrics_list.l1tex__t_sector_pipe_tex_mem_texture_op_tex_hit_rate = value; }
        if (metric_name == "smsp__sass_average_data_bytes_per_wavefront_mem_shared_op_ld.pct") { kernel_obj.metrics_list.smsp__sass_average_data_bytes_per_wavefront_mem_shared_op_ld = value; }
        if (metric_name == "l1tex__t_sectors_pipe_lsu_mem_local_op_st.sum") { kernel_obj.metrics_list.l1tex__t_sectors_pipe_lsu_mem_local_op_st = value; }
        if (metric_name == "l1tex__t_sector_pipe_lsu_mem_local_op_st_hit_rate.pct") { kernel_obj.metrics_list.l1tex__t_sector_pipe_lsu_mem_local_op_st_hit_rate = value; }
        if (metric_name == "l1tex__t_sector_pipe_lsu_mem_global_op_st_hit_rate.pct") { kernel_obj.metrics_list.l1tex__t_sector_pipe_lsu_mem_global_op_st_hit_rate = value; }
        if (metric_name == "lts__t_sector_op_write_hit_rate.pct") { kernel_obj.metrics_list.lts__t_sector_op_write_hit_rate = value; }
        if (metric_name == "lts__t_sector_hit_rate.pct") { kernel_obj.metrics_list.lts__t_sector_hit_rate = value; }
        if (metric_name == "sm__sass_inst_executed_op_global_st.sum") { kernel_obj.metrics_list.sm__sass_inst_executed_op_global_st = value; }
        if (metric_name == "sm__sass_inst_executed_op_local_st.sum") { kernel_obj.metrics_list.sm__sass_inst_executed_op_local_st = value; }
        if (metric_name == "smsp__inst_executed_op_ldgsts.sum") { kernel_obj.metrics_list.smsp__inst_executed_op_ldgsts = value; }
    }
